...
// ── POST /api/login ────────────────────────────────────────────────────────
// force=false: 다른 기기 이미 로그인 중이면 OCCUPIED 반환
// force=true : 강제 로그인 (기존 기기 kick)
app.post('/api/login', async (c) => {
  try {
    const body = await c.req.json().catch(() => ({}))
    const { userId, deviceId, force, currentToken } = body
    if (!userId || !deviceId) {
...
// Bearer 토큰 + X-User-Id + X-Device-Id 헤더로 세션 검증
// 200: 정상 / 401: 세션 없음 / 403: 다른 기기에서 kick됨
