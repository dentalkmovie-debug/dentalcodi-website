(function(){
  ...
  function getLoginName() {
    ...
    try {
      var bs = window.__bs_imweb;
      if (bs) {
        /* ★ v5.10.13: 'name' 추가 - 아임웹 최상위 이름 필드 */
        var fields = ['member_name','memberName','nick','nickname','user_name','name'];
  ...
  var SYSWORDS = ['마이페이지','로그인','회원가입','로그아웃','Logout','Login','Sign Up','My Page','MyPage',
    ...
    '멤버십관리','멤버십','모바일코디','모바일 코디','대기실TV','대기실 TV','임플란트코디','임플란트 코디',
    '관리자','최고관리자','치과관리','사이트관리','사이트','포인트관리시스템'];
  ...
  function doMatch(memberId, loginName) {
    ...
    try {
      ...
      /* ★ v5.10.12: 닉네임(loginName) 우선 적용 - title보다 정확함 */
      /* 닉네임 = 아임웹 회원 닉네임 = 치과명 (각 계정마다 고유) */
      /* title은 사이트 공통 제목이라 다른 계정 이름이 들어갈 수 있음 */
      if (!clinicName && loginName && !isSysWord(loginName)) clinicName = loginName;
      /* ★ title 태그는 loginName 없을 때만 사용 (isSysWord 필터 적용) */
      if (!clinicName) {
