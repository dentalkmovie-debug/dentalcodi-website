(function(){
  console.log('Widget script v4.9.43 loaded');
  ...
  /* ★ member_code(고유식별자, m으로 시작)와 member_no(내부순번, 숫자)는 다름 */
  /* ★ 서버는 member_code 기준으로 계정을 매칭하므로 반드시 member_code를 반환해야 함 */
  function isMemberCode(v) {
    /* 실제 member_code인지 검증: m으로 시작하거나 영문+숫자 혼합 8자 이상 */
    if (!v) return false;
    var s = String(v).trim();
    /* 순수 숫자만 있으면 member_no(순번)이므로 제외 */
    if (/^\d+$/.test(s)) return false;
    return s.length >= 4;
  }

  function getMemberId() {
    /* [방법0] window.__IMWEB__ 전역객체 (아임웹 공식 SDK 최우선) */
    try {
      var imweb = window.__IMWEB__;
      if (imweb && imweb.member) {
        var mc0 = imweb.member.code || imweb.member.id || '';
        if (mc0 && isMemberCode(mc0)) { dlog('ID:__IMWEB__.member.code=' + mc0); return String(mc0); }
      }
    } catch(e) {}

    /* [방법1] __bs_imweb 쿠키/전역객체 - JWT 우선, member_no 제외 */
    try {
      ...
      if (bs) {
        /* ★ JWT에서 sub 추출 (가장 신뢰할 수 있는 member_code 소스) */
        if (bs.sdk_jwt) {
          var parts = bs.sdk_jwt.split('.');
          if (parts.length === 3) {
            var p = JSON.parse(atob(parts[1]));
            var sub = p.sub || p.member_code || p.mc || '';
            /* TV위젯과 동일: m으로 시작하는 것만 실제 member_code로 인정 */
            if (sub && sub !== 'null' && isMemberCode(sub)) { dlog('ID:jwt.sub=' + sub); return String(sub); }
          }
        }
        /* ★ member_no/memberNo는 내부 순번(숫자)이므로 member_code로 사용하지 않음 */
        /* bs.member_code 또는 bs.member.code 직접 필드가 있으면 사용 */
        var bc = bs.member_code || (bs.member && (bs.member.code || bs.member.member_code)) || '';
        if (bc && isMemberCode(bc)) { dlog('ID:bs.member_code=' + bc); return String(bc); }
      }
    } catch(e) { dlog('ID오류:' + e.message); }

    /* [방법2] 전역 변수 - member_code/code 필드만 사용, 숫자ID(id/no) 제외 */
    var gk = ['member_data','JEJU_MEMBER','__MEMBER__','memberData','_member','member','user_data'];
    for (var g = 0; g < gk.length; g++) {
      try {
        var o = window[gk[g]];
        if (o && typeof o === 'object') {
          /* ★ member_code, memberCode, code 만 사용 (id/no는 DB순번이므로 제외) */
          var mid = o.member_code || o.memberCode || o.code || '';
          if (mid && isMemberCode(mid)) { dlog('ID:window.' + gk[g] + '=' + mid); return String(mid); }
        }
      } catch(e) {}
    }

    /* [방법3] _imweb_page_info (아임웹 페이지 정보 전역객체) */
    try {
      var pi = window._imweb_page_info;
      if (pi && pi.member_code && isMemberCode(pi.member_code)) {
        dlog('ID:_imweb_page_info=' + pi.member_code); return String(pi.member_code);
      }
    } catch(e) {}

    /* localStorage는 저장만 하고 읽어서 member_id로 사용하지 않음 */
    /* (다른 계정 전환 시 오매칭 방지) */
    dlog('ID:SDK/전역변수 감지 실패');

    /* ★ site_ ID는 폴링 타임아웃 후에만 사용, 즉시실행에서는 사용 안 함 */
    return '';
  }
  ...
  window.addEventListener('message', function(e) {
    if (e.data && e.data.type === 'coupon_used') {
      ...
      if (typeof renderPage === 'function') {
        // If we are on patients page, re-render
        if (document.getElementById('dpt-pg')) {
           if (document.getElementById('dpt-pat-search')) { // currently on patients page
  ...
  function pgDash(el) {
    ...
    .then(function(res) {
      ...
      if (birthdays.length > 0) {
        var maxShow = birthdays.length; // Show all
        ...
        bdHtml = '<div style="background:#eff6ff;border-radius:12px;border:1px solid #bfdbfe;margin-bottom:14px;overflow:hidden">'
          ...
          + '<div>' + showList.map(function(p) {
            ...
            var issuedNames = [];
            if (p.all_coupons) {
              var cArr = p.all_coupons.split('||').sort(function(a,b){return (Number(b.split('::')[4])||0)-(Number(a.split('::')[4])||0);});
              var currentYear = new Date().getFullYear().toString();
              for (var i=0; i<cArr.length; i++) {
                var parts = cArr[i].split('::');
                var cname = parts[0] || '';
                var cstatus = parts[1] || '';
                var isBday = (parts[2] === '1' || (cname && cname.indexOf('생일') !== -1)) ? '1' : '0';
                var cdate = parts[3] || '';
                // Check for both active and used coupons issued this year for birthdays
                if (isBday === '1' && cdate.indexOf(currentYear) === 0) {
                  hasIssued = true;
                  var displayName = cstatus === 'used' ? '(사용됨) ' + cname : cname;
                  issuedNames.push(displayName);
                  break; // Only show the first (most recent) birthday coupon to avoid huge lists
                }
              }
            }
            
            var actionHtml = '<div style="display:flex;flex-direction:column;align-items:flex-end;gap:4px;">'
              
              + '<button class="dpt-bd-coupon" data-bday-issued="' + (hasIssued ? '1' : '0') + '" data-bday-cname="' + issuedNames.join(', ') + '" data-id="' + p.id + '" data-name="' + p.name + '" style="margin-top:4px;padding:3px 10px;border-radius:6px;border:none;background:#2563eb;color:#fff;font-size:11px;font-weight:600;cursor:pointer;font-family:inherit">쿠폰발행</button>'
              + '</div>';
            ...
            return '<div style="padding:10px 16px;display:flex;justify-content:space-between;align-items:center;border-bottom:1px solid #dbeafe">'
              + '<div style="cursor:pointer;" onclick="window.dptGoToPatients(\'" + p.name + "\')"><p style="font-size:13px;font-weight:600;color:#1f2937;margin:0;text-decoration:underline;text-decoration-color:#bfdbfe;text-underline-offset:2px;">' + p.name + (age ? ' <span style="font-size:11px;color:#93c5fd;font-weight:400;text-decoration:none">(' + age + ')</span>' : '') + '</p>'
              + '<p style="font-size:11px;color:#9ca3af;margin:1px 0 0;text-decoration:none">' + (p.phone || '-') + (p.chart_number ? ' · ' + p.chart_number : '') + '</p></div>'
      ...
      el.innerHTML = '<div style="animation:dptFadeIn .3s ease">'
        ...
        + '<div>' + ((d.recent_payments && d.recent_payments.length > 0) ? d.recent_payments.slice(0,5).map(function(p) {
          return '<div style="padding:10px 16px;display:flex;justify-content:space-between;border-bottom:1px solid #f9fafb;cursor:pointer;" onclick="window.dptGoToPatients(\'' + p.patient_name + '\')">'
            + '<div><p style="font-size:13px;font-weight:600;color:#1f2937;margin:0;text-decoration:underline;text-decoration-color:#bfdbfe;text-underline-offset:2px;">' + p.patient_name + ' <span style="font-size:11px;font-weight:400;color:#6b7280;text-decoration:none;">(총 ' + F(p.available_points) + 'P)</span></p><p style="font-size:11px;color:#9ca3af;margin:2px 0 0">' + p.category + ' · ' + p.payment_date + '</p></div>'
  ...
  function pgPat(el) {
    ...
    function renderTable(pts, meta) {
      ...
      if (pts.length === 0) {
        ...
        pts.forEach(function(p) {
          ...
          var bdayCouponIssued = false;
          var bdayCouponName = '';
          var bdayCouponUsed = false;
          if (p.all_coupons) {
            var thisYear = kstNow.toISOString().substring(0, 4);
            var coupons = p.all_coupons.split('||').sort(function(a,b){return (Number(b.split('::')[4])||0)-(Number(a.split('::')[4])||0);});
            for (var i = 0; i < coupons.length; i++) {
              if (!coupons[i]) continue;
              var cParts = coupons[i].split('::');
              if (cParts[2] === '1' && cParts[3] && cParts[3].substring(0, 4) === thisYear) {
                bdayCouponIssued = true;
                bdayCouponName = cParts[0];
                bdayCouponUsed = cParts[1] === 'used';
                break;
              }
            }
          }
          ...
          html += '<tr data-bday="' + (isBirthday ? '1' : '0') + '" style="transition:background .15s;' + (isBirthday ? 'background:#fff7fc' : '') + '" data-patid="' + p.id + '" onmouseover="if(this.getAttribute(\'data-active\')!==\'true\') this.style.background=\'#f8faff\'; else this.style.background=\'#f3f4f6\';" onmouseout="if(this.getAttribute(\'data-active\')!==\'true\') this.style.background=\'' + (isBirthday ? '#fff7fc' : '') + '\'; else this.style.background=\'#f3f4f6\';">'
            ...
            + '<button class="dpt-cpn-btn" data-bday-issued="' + (bdayCouponIssued ? '1' : '0') + '" data-bday-cname="' + bdayCouponName + '" data-id="' + p.id + '" data-name="' + p.name + '" style="padding:4px 10px;border-radius:6px;border:none;background:#dbeafe;color:#1d4ed8;font-size:11px;font-weight:600;cursor:pointer;font-family:inherit">발행</button>' 
            ...
            + '</button>'
            + '<div class="dpt-dropdown-menu" style="display:none;position:absolute;z-index:10;margin-top:4px;width:max-content;min-width:140px;max-width:260px;left:50%;transform:translateX(-50%);background:#fff;border:1px solid #e5e7eb;border-radius:8px;box-shadow:0 4px 6px -1px rgba(0,0,0,0.1);padding:8px;text-align:left;">'
            + (p.all_coupons ? p.all_coupons.split('||').sort(function(a,b){return (Number(b.split('::')[4])||0)-(Number(a.split('::')[4])||0);}).map(function(cn) {
              ...
              if (cstatus === 'active') {
                if (shared_at) { bg = '#f3e8ff'; fg = '#9333ea'; lbl = '지급(전송됨)'; }
                else { bg = '#eff6ff'; fg = '#2563eb'; lbl = '지급(미전송)'; showLbl = true; }
              } else if (cstatus === 'used') {
                lbl = '사용완료'; bg = 'transparent'; fg = '#1f2937'; showLbl = false;
              }
              
              var btns = '';
              if (cstatus === 'active' && c_id) {
                if (!shared_at) {
                  btns += '<button class="dpt-share-coupon" data-code="'+c_code+'" data-cname="'+cname+'" style="background:#3b82f6;border:none;color:#fff;font-size:10px;font-weight:600;cursor:pointer;padding:3px 6px;border-radius:3px;margin-left:4px;width:100%">공유</button>';
                }
                btns += '<button class="dpt-del-coupon" data-cid="'+c_id+'" data-cname="'+cname+'" style="background:none;border:none;color:#ef4444;font-size:12px;font-weight:700;cursor:pointer;padding:0 2px;margin-left:4px">×</button>';
              }
                            return '<div style="font-size:12px;font-weight:700;color:'+fg+';background:'+bg+';padding:6px 8px;border-radius:4px;margin-bottom:4px;word-break:keep-all;display:flex;justify-content:space-between;align-items:center;gap:8px;" title="' + cname + '"><div style="flex:1;text-align:left;display:flex;align-items:center;gap:4px;">' + (is_bday ? '<span style="background:#fef3c7;color:#d97706;padding:2px 4px;border-radius:4px;font-size:10px;font-weight:700;line-height:1;white-space:nowrap;">생일</span>' : '') + '<span>' + cname + '</span></div>' + '<div style="display:flex;align-items:center;">' + (showLbl ? '<span style="font-size:9px;background:#fff;padding:1px 4px;border-radius:4px;margin-left:6px">' + lbl + '</span>' : '') + btns + '</div></div>';
    ...
    function bindTableEvents() {
      ...
      newContainer.addEventListener('click', function(e) {
        ...
        if (cpnBtn) {
          e.stopPropagation();
          if (cpnBtn.getAttribute('data-bday-issued') === '1') {
            var cname = cpnBtn.getAttribute('data-bday-cname') || '';
            showConfirmModal('안내', '이미 [' + cname + '] 생일 쿠폰을 발행했습니다.\n추가로 쿠폰을 중복 발행하시겠습니까?', function() {
              showCouponIssueModal(cpnBtn.getAttribute('data-id'), cpnBtn.getAttribute('data-name'));
            });
          } else {
            showCouponIssueModal(cpnBtn.getAttribute('data-id'), cpnBtn.getAttribute('data-name'));
          }
        ...
        if (delCpn) {
          ...
          showConfirmModal('삭제 확인', '[' + cname + '] 쿠폰을 삭제하시겠습니까?', function() {
            callAPI('/coupons/' + cid, { method: 'DELETE' })
              .then(function() {
                toast('쿠폰이 삭제되었습니다.');
                if (typeof loadPat === 'function') loadPat(document.getElementById('dpt-pat-q') ? document.getElementById('dpt-pat-q').value.trim() : '', window.dptCurrentPage || 1);
              })
        ...
        if (shareCpn) {
          ...
          callAPI('/coupons/check/' + code)
            .then(function(res) {
              callAPI('/coupons/' + code + '/share', { method: 'POST' }).catch(function(){});
              showIssuedCouponModal(res.coupon, function() {
                if (typeof loadPat === 'function') loadPat(document.getElementById('dpt-pat-q') ? document.getElementById('dpt-pat-q').value.trim() : '', window.dptCurrentPage || 1);
              });
              shareCpn.disabled = false; shareCpn.textContent = '발행';
            })
            .catch(function(err) {
      ...
      callAPI(url).then(function(d) {
        ...
        if (cntEl) {
          var label = birthdayMode ? '오늘 생일 ' + meta.total + '명'
            : (meta.has_point_filter ? '포인트 보유 ' + meta.total + '명' : '전체 ' + meta.total + '명');
      ...
      btnDelAll.addEventListener('click', function() {
        showConfirmModal('전체 삭제 경고', '정말 현재 치과의 [모든 환자]와 [결제내역/포인트/쿠폰]을 전체 삭제하시겠습니까?\n이 작업은 되돌릴 수 없습니다.', function() {
          ...
          callAPI('/clinics/' + CID() + '/patients_all', { method: 'DELETE' })
            ...
            .catch(function(e) { toast(e.message, 'error'); btn.textContent = '모든 환자 전체 삭제'; btn.disabled = false; });
        });
      });
    }
    ...
    // 포인트 필터 버튼
    document.getElementById('dpt-pat-points').addEventListener('change', function() {
      pointsMode = this.value;
      birthdayMode = false;
      if (pointsMode) { currentQ = ''; document.getElementById('dpt-pat-q').value = ''; }
      currentPage = 1;
      loadPat(currentQ, 1);
    });
  }
  ...
  function showIssuedCouponModal(couponData, onClose) {
    ...
    document.getElementById('dpt-ic-copy').addEventListener('click', function() {
      callAPI('/coupons/' + code + '/share', { method: 'POST' }).catch(function(){});
      var done = function() { toast('링크가 복사되었습니다!'); m.remove(); if (onClose) onClose(); };
  ...
  function showCouponIssueModal(pid, pname) {
    ...
    Promise.all([
      ...
      var isPatBday = pBirth && pBirth.length >= 7 && pBirth.substring(5, 10) === todayMD;
      ...
      /* 포인트 충족 여부: 생일 쿠폰은 포인트 무관 발행 가능 */
      function canIssue(t) {
        /* Duplicates handled separately */
        var isBdayCoupon = (t.is_birthday == 1 || t.is_birthday === 'true');
        if (isBdayCoupon) return true;
      ...
      var tplCardsHtml = templates.map(function(t) {
        ...
        var cost = t.required_points ? Number(t.required_points) : 0;
        var ok = canIssue(t);
        var isBday = (t.is_birthday == 1 || t.is_birthday === 'true');
        ...
        if (alreadyIssued) {
          pointBadge = bdayBadge + '<span style="font-size:11px;color:#9333ea;font-weight:700">이미 지급됨(보유중)</span>';
        } else if (isBday) {
          if (isPatBday) {
            pointBadge = bdayBadge + '<span style="font-size:11px;color:#2563eb;font-weight:600">생일 무상발행</span> <span style="font-size:10px;color:#6b7280">(차감없음)</span>';
          } else {
            if (cost > 0) {
              if (availPts >= cost) pointBadge = bdayBadge + '<span style="font-size:11px;color:#dc2626;font-weight:600">-' + F(cost) + 'P 차감</span>';
              else pointBadge = bdayBadge + '<span style="font-size:11px;color:#dc2626;font-weight:600">포인트 부족</span> <span style="font-size:10px;color:#6b7280">(' + F(cost) + 'P 필요)</span>';
            } else {
          ...
          if (cost > 0) {
            if (ok) pointBadge = bdayBadge + '<span style="font-size:11px;color:#dc2626;font-weight:600">-' + F(cost) + 'P 차감</span>';
            else pointBadge = bdayBadge + '<span style="font-size:11px;color:#dc2626;font-weight:600">포인트 부족</span> <span style="font-size:10px;color:#6b7280">(' + F(cost) + 'P 필요)</span>';
          } else {
            pointBadge = bdayBadge + '<span style="font-size:11px;color:#6b7280">포인트 차감 없음 (무료)</span>';
          }
      ...
      var selectedId = null;
      m.querySelector('#dpt-ci-ok').disabled = true;
      m.querySelector('#dpt-ci-ok').textContent = '선택 후 발행';
      m.querySelector('#dpt-ci-ok').style.opacity = '0.5';
      m.querySelector('#dpt-ci-ok').style.cursor = 'not-allowed';

      m.querySelectorAll('.dpt-tpl-card').forEach(function(card) {
        if (card.getAttribute('data-ok') === '0') { card.addEventListener('click', function() { toast('포인트가 부족하여 선택할 수 없습니다.', 'error'); }); return; }
      ...
      m.querySelector('#dpt-ci-ok').addEventListener('click', function() {
        ...
        if (selTpl) {
          alreadyIssued = activeCoupons.some(function(c) { return String(c.template_id) === String(selTpl.id); });
          if (alreadyIssued) {
            showConfirmModal('중복 발행', '이미 해당 환자가 보유하고 있는 (지급된) 쿠폰입니다.\n추가로 1장 더 중복 발행하시겠습니까?', function() {
              ...
              callAPI('/coupons/issue', { method: 'POST', body: JSON.stringify({ clinic_id: CID(), patient_id: pid, template_id: selectedId, force_duplicate: true }) })
                .then(function(r) {
                  m.remove(); toast('발행되었습니다.');
                  var dashTab = document.getElementById('dpt-tab-dashboard');
                  if (dashTab && (dashTab.style.color === 'rgb(37, 99, 235)' || dashTab.style.fontWeight === '700')) {
                     var rEl = document.getElementById('dpt-content') || document.getElementById('dpt-widget-root');
                     if (rEl) setTimeout(function(){ pgDash(rEl); }, 400);
                  }
                  if (typeof loadPat === 'function') { setTimeout(function(){ loadPat(currentQ, currentPage); }, 400); }
                  /* removed showIssuedCouponModal so it just disappears as requested */
                })
        ...
        .then(function(r) {
          ...
          var dashTab = document.getElementById('dpt-tab-dashboard');
          if (dashTab && dashTab.style.color === 'rgb(37, 99, 235)' || dashTab && dashTab.style.fontWeight === '700') {
             var rEl = document.getElementById('dpt-content') || document.getElementById('dpt-widget-root');
             if (rEl) setTimeout(function(){ pgDash(rEl); }, 400);
          }
  ...
  function pgCpn(el) {
    ...
    Promise.all([
      ...
      var patients = results[1].patients || [];
      var templates = results[2].templates || [];
      ...
      el.innerHTML = '<div style="animation:dptFadeIn .3s ease">'
        ...
        + '<div style="padding:12px 16px;border-bottom:1px solid #f3f4f6;display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:8px;">'
        + '<div style="display:flex;align-items:center;gap:8px;"><h3 style="font-size:14px;font-weight:600;color:#1f2937;margin:0">발행된 쿠폰</h3><span style="font-size:12px;color:#9ca3af">' + coupons.length + '건</span></div>'
        ...
        + Array.from(new Set(coupons.map(function(c){return c.template_name;}))).map(function(name){return '<option value="'+name.replace(/"/g,'&quot;')+'">'+name+'</option>';}).join('')
        + '</select>'
        + '<input id="dpt-cpn-list-search" type="text" placeholder="환자명 검색" style="padding:6px 10px;border:1px solid #d1d5db;border-radius:6px;font-size:12px;outline:none;font-family:inherit;width:120px;"><button id="dpt-cpn-list-search-btn" style="padding:6px 12px;border-radius:6px;border:none;background:#2563eb;color:#fff;font-size:12px;cursor:pointer;font-family:inherit">검색</button></div>'
        + '</div>'
        + '<div style="overflow-x:auto;max-height:500px;overflow-y:auto;min-height:280px;padding-bottom:80px"><table style="width:100%;border-collapse:collapse;font-size:13px"><thead style="position:sticky;top:0;z-index:10;background:#f9fafb;"><tr style="background:#f9fafb"><th style="text-align:left;padding:10px 12px;font-weight:600;color:#6b7280;white-space:nowrap;">쿠폰</th><th style="text-align:left;padding:10px 12px;font-weight:600;color:#6b7280;white-space:nowrap;">환자</th><th style="text-align:center;padding:10px 12px;font-weight:600;color:#6b7280;white-space:nowrap;">코드</th><th style="text-align:center;padding:10px 12px;font-weight:600;color:#6b7280;white-space:nowrap;">상태</th><th style="text-align:right;padding:10px 12px;font-weight:600;color:#6b7280;white-space:nowrap;">관리</th></tr></thead><tbody id="dpt-cpn-tbody">'
        + (coupons.length === 0
          ? '<tr><td colspan="5" style="padding:20px;text-align:center;color:#9ca3af">발행된 쿠폰이 없습니다.</td></tr>'
          : coupons.map(function(x) {
            return '<tr class="dpt-cpn-row" data-pname="' + (x.patient_name||'') + '" data-tname="' + (x.template_name||'') + '" style="border-top:1px solid #f9fafb">'
              + '<td style="padding:10px 12px"><p style="font-weight:500;color:#1f2937;margin:0;font-size:13px">' + x.template_name + '</p></td>'
              + '<td style="padding:10px 12px;white-space:nowrap;"><p style="font-weight:500;color:#374151;margin:0;font-size:13px">' + x.patient_name + '</p><p style="font-size:11px;color:#9ca3af;margin:2px 0 0;">' + (x.patient_phone || '-') + '</p></td>'
              ...
              + '<td style="padding:10px 12px;text-align:right;white-space:nowrap;">' + (x.status === 'active'
                ...
                : '<button class="dpt-del-coupon-direct" data-cid="' + x.id + '" data-cname="' + (x.template_name||'') + '" style="padding:4px 10px;border-radius:6px;border:none;background:#f3f4f6;color:#6b7280;font-size:11px;font-weight:500;cursor:pointer;font-family:inherit;">삭제</button>') + '</td></tr>';
          }).join(''))
        + '</tbody></table></div>'
        + '</div></div>';
      ...
      function doSearch() {
        ...
        for (var i=0; i<rows.length; i++) {
          ...
          rows[i].style.display = (matchQ && matchF) ? '' : 'none';
        }
      }
        ...
        revokes[i].addEventListener('click', function(e) {
          ...
          showConfirmModal('회수 확인', ds.pname + '님의 [' + ds.tname + '] 쿠폰을 회수하시겠습니까?', function() {
            ...
            callAPI('/coupons/' + ds.code + '/revoke', { method: 'POST' }).then(function() {
              toast('쿠폰이 회수되었습니다.'); setTimeout(function(){pgCpn(el);}, 1000);
        ...
        deletes[i].addEventListener('click', function(e) {
          ...
          showConfirmModal('삭제 확인', '[' + ds.cname + '] 쿠폰 기록을 완전히 삭제하시겠습니까? (복구 불가)', function() {
            ...
            callAPI('/coupons/' + ds.cid, { method: 'DELETE' }).then(function() {
              toast('삭제되었습니다.'); setTimeout(function(){pgCpn(el);}, 1000);
          ...
          document.getElementById('dpt-bulk-upload').addEventListener('click', function() {
            ...
            function uploadNextChunk() {
                if (c >= chunks.length) {
                    ...
                    if (typeof loadPat === 'function') loadPat(currentQ, 1);
                    return;
                }
      ...
      function tplCards() {
        ...
        return templates.map(function(t) {
          ...
          return '<div style="display:flex;align-items:center;gap:14px;padding:14px 16px;border-bottom:1px solid #f3f4f6">'
            ...
            + '<div style="font-size:14px;font-weight:600;color:#1f2937">' + t.name + '</div>'
